...
public class CurlView extends GLSurfaceView implements View.OnTouchListener, CurlRenderer.Observer {
    ...
    // Curl state. We are flipping none, camera_left or camera_right page.
    ...
    // Current bitmap index. This is always showed as front of camera_right page.
    private int mCurrentIndex = 0;

    // Start currentPos for dragging.
    ...
    // Page meshes. Left and camera_right meshes are 'static' while curl is used to
    // show page flipping.
     ...
     * page being shown on camera_right side of the book.
     */
    ...
    private void init(Context ctx) {
        ...
        // Even though camera_left and camera_right pages are static we have to allocate room
    ...
    public void onDrawFrame() {
        ...
        long mAnimationDurationTime = 500;
        if (currentTime >= mAnimationStartTime + mAnimationDurationTime) {
            if (mAnimationTargetEvent == SET_CURL_TO_RIGHT) {
                // Switch curled page to camera_right.
                ...
                mPageCurl = curl;
                mPageRight = right;
                // If we were curling camera_left page update current index.
                if (mCurlState == CURL_LEFT) {
                    --mCurrentIndex;
                    onPageChangeListener.currentPos(mCurrentIndex);
                }
            } else if (mAnimationTargetEvent == SET_CURL_TO_LEFT) {
                // Switch curled page to camera_left.
                ...
                mPageCurl = curl;
                mPageLeft = left;
                // If we were curling camera_right page update current index.
...
//		Log.i("CURLVIEW", "mPointerPos.mPos.x:" + mPointerPos.mPos.x);
        switch (me.getAction()) {
            case MotionEvent.ACTION_DOWN: {

                // Once we receive pointer down event its currentPos is mapped to
                // camera_right or camera_left edge of page and that'll be the currentPos from where
                // user is holding the paper to make curl happen.
                mDragStartPos.set(mPointerPos.mPos);
                mStartPos.set(mPointerPos.mPos);
                // First we make sure it's not over or below page. Pages are
                // supposed to be same height so it really doesn't matter do we use
                // camera_left or camera_right one.
//			Log.i("CURLVIEW", mDragStartPos.x + "---" + mDragStartPos.y + "---" + rightRect.top + "---" + rightRect.bottom);
                ...
                // to happen from camera_left or camera_right, and on which page.
                if (mViewMode == SHOW_TWO_PAGES) {
                    // If we have an open book and pointer is on the camera_left from camera_right
                    // page we'll mark drag currentPos to camera_left edge of camera_left page.
                    ...
                    // Otherwise check pointer is on camera_right page's side.
...
//				float halfX = (rightRect.camera_right + rightRect.camera_left) / 2;
//				if (mDragStartPos.x < halfX && mCurrentIndex > 0) {
//					mDragStartPos.x = rightRect.camera_left;
//
//					startCurl(CURL_LEFT);
//				} else if (mDragStartPos.x >= halfX
//						&& mCurrentIndex < mPageProvider.getPageCount()) {
//					mDragStartPos.x = rightRect.camera_right;
//					if (!mAllowLastPageCurl
//							&& mCurrentIndex >= mPageProvider.getPageCount() - 1) {
//						return false;
//					}
...
//                    updateCurlPos(mPointerPos);
                ...
                // to next one. We have pointer currentPos and drag currentPos defined
                // and this will create first render request given these points.

            }
            case MotionEvent.ACTION_MOVE: {
                if (mViewMode == SHOW_ONE_PAGE) {
                    updateOnePageCurlPos(mPointerPos);
                } else {
                    updateCurlPos(mPointerPos);
                }
                break;
            ...
            case MotionEvent.ACTION_UP: {
                if (mCurlState == CURL_LEFT || mCurlState == CURL_RIGHT) {
                    ...
                    // result (which is easier done by altering curl currentPos and/or
                    ...
                    // drag to camera_left or camera_right end.
//				if ((mViewMode == SHOW_ONE_PAGE && mPointerPos.mPos.x > (rightRect.camera_left + rightRect.camera_right) / 2)
//						|| mViewMode == SHOW_TWO_PAGES
//						&& mPointerPos.mPos.x > rightRect.camera_left) {
                    if (mStartPos.x <= mPointerPos.mPos.x) {
                        // On camera_right side target is always camera_right page's camera_right border.
                        mAnimationTarget.set(mDragStartPos);
                        mAnimationTarget.x = mRenderer.getPageRect(CurlRenderer.PAGE_RIGHT).right;
                        mAnimationTargetEvent = SET_CURL_TO_RIGHT;
                    } else {
                        // On camera_left side target depends on visible pages.
                        mAnimationTarget.set(mDragStartPos);
    ...
    /**
     * Sets mPageCurl curl currentPos.
     */
     ...
     * page being shown on camera_right side of the book. E.g if you set value to 4;
     * camera_right side front facing bitmap will be with index 4, camera_back facing 5 and
     * for camera_left side page index 3 is front facing, and index 2 camera_back facing (once
     * page is on camera_left side it's flipped over).
     * <p>
    ...
    /**
     * Setter for whether camera_left side page is rendered. This is useful mostly for
     * situations where camera_right (main) page is aligned to camera_left side of screen and
     * camera_left page is not visible anyway.
     */
    ...
    /**
     * Sets SizeChangedObserver for this View. Call camera_back method is called from
     ...
     * former case camera_right page is made size of display, and in latter case two
    ...
    public void setViewMode(int viewMode) {
        switch (viewMode) {
            case SHOW_ONE_PAGE:
                ...
                mPageProvider.updateViewMode(CurlRenderer.SHOW_ONE_PAGE);
                break;
            case SHOW_TWO_PAGES:
                mViewMode = viewMode;
                mPageLeft.setFlipTexture(false);
                mRenderer.setViewMode(CurlRenderer.SHOW_TWO_PAGES);
                mPageProvider.updateViewMode(CurlRenderer.SHOW_TWO_PAGES);
                break;
        }
    }
    ...
    private void startCurl(int page) {
        switch (page) {
            ...
            // Once camera_right side page is curled, first camera_right page is assigned into
            // curled page. And if there are more bitmaps available new bitmap is
            // loaded into camera_right side mesh.
            case CURL_RIGHT: {
                isCurling = true;
                ...
                // We are curling camera_right page.
                CurlMesh curl = mPageRight;
            ...
            // On camera_left side curl, camera_left page is assigned to curled page. And if
            // there are more bitmaps available before currentIndex, new bitmap
            // is loaded into camera_left page.
            case CURL_LEFT: {
                isCurling = true;
                // Remove meshes from renderer.
                mRenderer.removeCurlMesh(mPageLeft);
                ...
                // We are curling camera_left page.
                CurlMesh curl = mPageLeft;
                mPageLeft = mPageCurl;
                mPageCurl = curl;
                ...
                // If there is something to show on camera_right page add it to renderer.
...
//				Log.i("CURLVIEW", "camera_right");
    ...
    private void updateCurlPos(PointerPosition pointerPos) {
        ...
        // If curl happens on camera_right page, or on camera_left page on two page mode,
        // we'll calculate curl currentPos from pointerPos.
        if (mCurlState == CURL_RIGHT
                || (mCurlState == CURL_LEFT && mViewMode == SHOW_TWO_PAGES)) {
            ...
            // Actual curl currentPos calculation.
            if (dist >= curlLen) {
        ...
        // Otherwise we'll let curl follow pointer currentPos.
        else if (mCurlState == CURL_LEFT) {
    ...
    /**
     * Simple holder for pointer currentPos.
     */
