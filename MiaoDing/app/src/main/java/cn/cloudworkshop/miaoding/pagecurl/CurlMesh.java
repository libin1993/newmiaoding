...
class CurlMesh {
	...
	// curl currentPos and one for the direction from the
	// currentPos given. Comes handy once playing around with different
	...
	// Bounding rectangle for this mesh. mRectagle[0] = top-camera_left corner,
	// mRectangle[1] = bottom-camera_left, mRectangle[2] = top-right and mRectangle[3]
	// bottom-right.
	...
	public CurlMesh(int maxCurlSplits) {
		...
		// Rectangle consists of 4 vertices. Index 0 = top-camera_left, index 1 =
		// bottom-camera_left, index 2 = top-camera_right and index 3 = bottom-camera_right.
		for (int i = 0; i < 4; ++i) {
	...
	public synchronized void curl(PointF curlPos, PointF curlDir, double radius) {
		...
		for (int i = 0; i < mArrScanLines.size(); ++i) {
			...
			while (mArrOutputVertices.size() > 0) {
				...
				// Move vertex camera_back to 'world' coordinates.
				v.rotateZ(curlAngle);
				v.translate(curlPos.x, curlPos.y);
				addVertex(v);
	...
	public synchronized void onDrawFrame(GL10 gl) {
		...
		// Draw camera_back facing blank vertices.
		gl.glDrawArrays(GL10.GL_TRIANGLE_STRIP, backStartIdx, backCount);

		// Draw camera_back facing texture.
		if (DRAW_TEXTURE) {
			gl.glEnable(GL10.GL_BLEND);
			gl.glEnable(GL10.GL_TEXTURE_2D);
