 ...
 * @author harism
 */
class CurlRenderer implements GLSurfaceView.Renderer {

	// Constant for requesting camera_left page rect.
	static final int PAGE_LEFT = 1;
	// Constant for requesting camera_right page rect.
	static final int PAGE_RIGHT = 2;
	// Constants for changing view mode.
	static final int SHOW_ONE_PAGE = 1;
	static final int SHOW_TWO_PAGES = 2;
	...
	/**
	 * Returns rect reserved for camera_left or camera_right page. Value page should be
	...
	private void updatePageRects() {
		if (mViewRect.width() == 0 || mViewRect.height() == 0) {
			...
			Log.i("CURLVIEW", mViewRect.width() + "***" + mViewRect.height());
			Log.i("CURLVIEW", mMargins.left + "***" + mMargins.right + "***" + mMargins.top + "***" + mMargins.bottom);
			Log.i("CURLVIEW", mPageRectRight.left + "***" + mPageRectRight.right + "***" + mPageRectRight.top + "***" + mPageRectRight.bottom);
